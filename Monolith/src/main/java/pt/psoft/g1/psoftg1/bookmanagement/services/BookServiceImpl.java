...
public class BookServiceImpl implements BookService {
    ...
    private final AgeThresholdProperties ageThresholdProperties;
    ...
    public Book create(CreateBookRequest request, String isbn) {
        ...
        final var genre = genreRepository.findByString(request.getGenre()).orElseThrow(() -> new NotFoundException("Genre not found"));
    ...
    public List<Book> recommendBooksByAge(int age, Long readerId) {
        if (age < ageThresholdProperties.getChild()) {
            return bookRepository.findByGenre_GenreIgnoreCase("children");
        } else if (age < ageThresholdProperties.getJuvenile()) {
            return bookRepository.findByGenre_GenreIgnoreCase("juvenile");
        } else {
            String mostLentGenre = lendingRepository.findMostLentGenreByReader(readerId);
